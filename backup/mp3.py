...
class Cat_Coord(object):
    def __init__(self, host, port):
        ...
        self.dl_detect_period = float(3000.0) #ms
    ...
    def exe_doCOMMIT_ABORT(self, MODE, TID):
        ...
        if MODE == "doCOMMIT":
            ...
            for each_obj in self.OBJ_DIC.values():
                ...
                #each_obj.theWaitList.print_EntryList(each_obj.name) # debug
                each_obj.theWaitList.delete_TID(TID, each_obj.name)
                #each_obj.theWaitList.print_EntryList(each_obj.name)  # debug

                each_obj.unlock(TID, "rw")  # Unlock all objects locked by This TID

                each_obj.theWaitList.delete_Abort_Skip(TID) # remove aborted TID from the skip list
        ...
        for req_TID in self.REQ_COMMIT_TID:
            canVote = True
            for each_name, each_obj in self.OBJ_DIC.iteritems():
                if (req_TID in each_obj.theWaitList.TID_cnt.keys()) or (req_TID in each_obj.theWaitList.Abort_Skip_TID):
                    canVote = False
                    break
...
class WaitList(object):
    ...
    def run_one_entry(self, my_indx, entry, host_name, host_obj, host_Server):
        ...
        if pre_myTID_exist:
            ...
            if entry.cmd == "SET":
                ...
                # Corner Case: GET: NOT FOUND and Abort
                if val_get == "NOT FOUND":
                    sys.stderr.write("Corner Case: GET: NOT FOUND and Abort\n")
                    self.Abort_Skip_TID.append(entry.tid)
                    if entry.tid in host_Server.REQ_COMMIT_TID:
                        host_Server.REQ_COMMIT_TID.remove(entry.tid)
    ...
    def run_ready_entry(self, host_name, host_obj, host_Server):
        ...
        for indx in range(first_ready_indx, len(self.EntryList)):
            if self.EntryList[indx].tid not in self.Abort_Skip_TID:
        ...
        # Corner Case: GET: NOT FOUND and Abort
        for abort_tid in self.Abort_Skip_TID:
            self.delete_TID(abort_tid, host_name)
            #if abort_tid in self.TID_cnt.keys():
            #    del self.TID_cnt[abort_tid]

        return 0

    def delete_Abort_Skip(self, TID):
        if TID in self.Abort_Skip_TID:
            self.Abort_Skip_TID.remove(TID)
